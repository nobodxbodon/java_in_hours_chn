...
## 没有规矩, 不成方圆 - 代码风格 (建议时间: >0 现在只要有这根筋就好,写了几年代码的人也许重温这里会有另外的感触)
...
- 导入语句都是全路径, 而不是通配符*. (好像没说过: import java.util.*就是导入所有java.util包下的类)
...
上述代码风格在Google Java代码风格指南的基础上, 加入了中文代码的特色部分. 删去了下面的部分. 请自行取舍:
- 包和导入语句都是单行 (是的,可以分行写)
